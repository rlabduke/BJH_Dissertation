      ...
      for residue_group in chain.residue_groups():
        for conformer in residue_group.conformers():
          for residue in conformer.residues():
            ...
            if residue.resname.strip() in ['DA','DG'] :
...
#       P2p = [p for p in ppeaks if p.adjacent_to('C4', C4_radii)]
  ...
  def has_super_strong_decoys(self): 
    return len(self.super_strong_decoys)

  def has_strong_decoys(self): 
    return len(self.strong_decoys)

  def has_weak_decoys(self): 
    return len(self.weak_decoys)
...
def run(args) :
  ...
  ddab.clean_up_files()
  ddab.write_potential_decoy_summary(log=sys.stdout)
